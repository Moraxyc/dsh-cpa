...
import { assertCpaBinary, DEFAULT_AUTH_FILES_TTL_MS, DEFAULT_DAILY_REQUEST_LIMIT, DEFAULT_PORT, DEFAULT_QUOTA_CONCURRENCY, DEFAULT_QUOTA_TTL_MS, DEFAULT_REFRESH_MS, DEFAULT_ROUTING_STRATEGY, fetchModels, positiveNumber, randomKey, spawnCpa, stopChild, waitForCpa, writeCpaSettings, writeManagedConfig, } from '../core/config.js';
...
function routingStrategy(value, fallback) {
...
function isConnectionError(cause) {
...
function modelAccounts(models, accounts) {
    const result = {};
...
function diagnosticsStatus(active, checks) {
    if (!active)
        return 'unavailable';
...
export function mergeCpaSettings(current, patch) {
    ...
    if (isString(patch.externalUrl))
        next.externalUrl = patch.externalUrl.trim();
    if (isString(patch.externalApiKey))
        next.externalApiKey = patch.externalApiKey;
    if (isString(patch.externalManagementKey))
        next.externalManagementKey = patch.externalManagementKey;
    if (isString(patch.internalBin))
        next.internalBin = patch.internalBin.trim();
    if (isBoolean(patch.usageStatisticsEnabled))
        next.usageStatisticsEnabled = patch.usageStatisticsEnabled;
    ...
    if (patch.refreshIntervalMs !== undefined)
        next.refreshIntervalMs = positiveNumber(scalar(patch.refreshIntervalMs), next.refreshIntervalMs);
    if (patch.port !== undefined)
        next.port = positiveInteger(scalar(patch.port), next.port);
    if (patch.authFilesTtlMs !== undefined)
        next.authFilesTtlMs = positiveNumber(scalar(patch.authFilesTtlMs), next.authFilesTtlMs);
    if (patch.quotaTtlMs !== undefined)
        next.quotaTtlMs = positiveNumber(scalar(patch.quotaTtlMs), next.quotaTtlMs);
    if (patch.quotaConcurrency !== undefined)
        next.quotaConcurrency = positiveInteger(scalar(patch.quotaConcurrency), next.quotaConcurrency);
...
export class CpaController {
    ...
    async diagnostics() {
        ...
        const errors = [];
        if (this.lastError)
            errors.push(this.lastError.slice(0, 200));
        if (quotaError)
    ...
    update(patch) {
        return this.enqueue(async () => {
            ...
            try {
                if (this.active && next.mode === this.settings.mode)
    ...
    installProjection() {
        if (this.disposeProjection !== undefined)
            return;
        try {
    ...
    startTimer() {
        this.stopTimer();
        if (!this.active || this.settings.refreshIntervalMs <= 0)
            return;
    ...
    async startInternal(next) {
        if (this.active && this.activeMode === 'internal')
            return;
        ...
        this.active = true;
        if (oldHandle !== undefined && oldHandle !== handle)
            await stopChild(oldHandle);
    }
    async startExternal(next) {
        if (this.active && this.activeMode === 'external' && cpaSettingsEqual(next, this.settings))
            return;
