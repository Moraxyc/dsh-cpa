...
function diagnosticsHandler(options) {
    return async (_req, res) => {
...
function preflightHandler(options) {
    return async (req, res) => {
        ...
        if (isNumber(value.inputTokens) && value.inputTokens >= 0)
            input.inputTokens = value.inputTokens;
        if (isNumber(value.maxTokens) && value.maxTokens > 0)
            input.maxTokens = value.maxTokens;
...
function reportHandler(options) {
    return async (req, res) => {
        if (req.method !== 'GET') {
            sendJson(res, 405, { error: 'method not allowed' });
            return;
